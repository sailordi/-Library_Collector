...
const QString DATA_SUFFIX = ".LibColDataV2",FILTER = "Library collector Data(*"+DATA_SUFFIX+")";
...
void WindowMain::updateSelectedBuildData() {
    int size = this->v_buildDataViewW->buildDataList().size(),selected = this->v_buildDataViewW->numberOfSelectedRows();
        ...
        try {
            ...
            int index = this->v_buildDataViewW->selectedRowsPosition().first();
...
void WindowMain::preformCollectionBtnClicked() {
    ...
    QList<BuildDataP> l = this->v_buildDataViewW->buildDataList();
        ...
        try {
            MessageHandler::errorCollectionPreform(outP,libraryBaseName,headerP,l.size() );
...
void WindowMain::saveProgramData() {
    QString str = QFileDialog::getSaveFileName(nullptr,"Save Library collection data..",QString(),FILTER);
...
void WindowMain::loadProgramData() {
    QString str = QFileDialog::getOpenFileName(nullptr,"Load Library collection data..",QString(),FILTER);
...
void WindowMain::saveWindowSettings() {
    Settings s(SETTINGS_PATH+SETTINGS_FILE);
