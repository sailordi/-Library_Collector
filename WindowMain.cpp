...
void WindowMain::preformCollectionBtnClicked() {
    ...
    QString outN = this->v_ui->libraryOutName_edit->text();
        ...
        Collector hR(in+SLASH,this->v_ui->libraryHeaderPath_edit->toPlainText()+SLASH,
        {"h"},true);
        Collector bR(lib+SLASH,this->v_ui->libraryReleasePath_edit->toPlainText()+SLASH,
        {"a","dll","lib","pdb"},false);
        Collector bD(lib+SLASH,this->v_ui->libraryDebugPath_edit->toPlainText()+SLASH,
        {"a","dll","lib","pdb"},false);
